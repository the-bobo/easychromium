...
# you need: XCode 7+, OSX 10.9+, ~10-20GB of space, ~3-5 hours (on 16GB RAM, more if less RAM)

# This script builds the latest release of the open source Chromium browser for OS X from source
...
echo "easychromium2.sh" | tee -a $LOGFILE
echo "=========New Build Attempt=========" | tee -a $LOGFILE
...
if [[ "$XCODE_CHECK" =~ "requires" ]]; then
	...
	for cmd in xcodebuild; do
		[[ $("$cmd" -version) =~ ([0-9][.][0-9.]*) ]] && version="${BASH_REMATCH[1]}"
		if ! awk -v ver="$version" 'BEGIN { if (ver < 7.0) exit 1; }'; then
			tput setaf 1
			echo 'ERROR: '$cmd' version 7.0 or higher required' | tee -a $LOGFILE
...
if [ -d /Applications/Chromium.app/ ]
  ...
  version_2=$CURRENT
	...
	if [[ $? -eq 0 ]]; then
		echo "TARGET version $TARGET is newer than CURRENT $CURRENT, proceeding to update" | tee -a $LOGFILE
	else
		echo "TARGET version $TARGET is NOT newer than CURRENT $CURRENT, aborting" | tee -a $LOGFILE
		exit 1;
	fi
else
...
if [ -d ./chromium/src/ ]
	then
	echo "./chromium/src/ found, proceeding to gclient sync master" | tee -a $LOGFILE
else
	echo "./chromium/src/ not found, assuming this is a fresh install, fetching chromium" | tee -a $LOGFILE
	echo "this will take many hours, possibly overnight depending on connection speed. ~50GB of data" | tee -a $LOGFILE
	tput sgr0
...
cd src
tput setaf 1
echo "updating master branch with git checkout master, git rebase-update, gclient-sync" | tee -a $LOGFILE
tput sgr0
git checkout master
git rebase-update
...
tput setaf 1
echo "grabbing tags with git fetch --tags origin" | tee -a $LOGFILE
tput sgr0
git fetch --tags origin 

tput setaf 1
echo "running gclient sync" | tee -a $LOGFILE
tput sgr0
gclient sync --verbose --verbose --verbose --jobs 16

tput setaf 1
echo "checking out new_release$TARGET branch" | tee -a $LOGFILE
tput sgr0
git checkout -b new_release$TARGET tags/$TARGET | tee -a $LOGFILE

tput setaf 1
echo "last gclient sync before building" | tee -a $LOGFILE
tput sgr0
gclient sync --verbose --verbose --verbose --with_branch_heads --jobs 16 | tee -a $LOGFILE

# last switch to master to sync now that we have branch_heads for the new_release$TARGET branch
...
tput setaf 1
echo "writing args.gn" | tee -a $LOGFILE
touch "args.gn"
...
tput setaf 1
echo "creating build directory with gn gen out/Default" | tee -a $LOGFILE
tput sgr0
gn gen out/Default 

tput setaf 1
echo "entering build with ninja -C out/Default chrome" | tee -a $LOGFILE
tput sgr0
